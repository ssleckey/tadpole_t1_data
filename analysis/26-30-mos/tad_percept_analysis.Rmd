...
```
There are `r length(levels(d.raw_sw$ID))` eyetracker participants with transitions data.

```{r}
d.raw_et$ID <- as.factor(as.character(d.raw_et$ID))
length(levels(d.raw_et$ID))
```
There are `r length(levels(d.raw_et$ID))` eyetracker participants with accuracy data.
...
There are `r length(levels(d_et$ID))` participants with both transitions and accuracy data.
