...
t.test(subset(mss_et_aov, trial_type=="inacc")$trans, 
       subset(mss_et_aov, trial_type=="similar_acc")$trans)

t.test(subset(mss_et_aov, trial_type=="inacc")$trans, 
       subset(mss_et_aov, trial_type=="dissimilar_acc")$trans)
...
```{r, echo=FALSE}
mss_et_aov <- mss_et_aov %>%
  group_by(trial_type)%>%
  multi_boot_standard(col = "trans", na.rm = TRUE)

ggplot(mss_et_aov, aes(x = factor(trial_type), y = mean, fill = trial_type)) + geom_bar(stat = "identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9)) +
  xlab("Trial Type") + 
...
t.test(subset(mss_ts_aov, trial_type=="inacc")$RT, 
       subset(mss_ts_aov, trial_type=="similar_acc")$RT)

t.test(subset(mss_ts_aov, trial_type=="inacc")$RT, 
       subset(mss_ts_aov, trial_type=="dissimilar_acc")$RT)
...
```{r, echo=FALSE}
mss_ts_aov <- mss_ts_aov %>%
  group_by(trial_type)%>%
  multi_boot_standard(col = "RT", na.rm = TRUE)

ggplot(mss_ts_aov, aes(x = factor(trial_type), y = mean, fill = trial_type)) + geom_bar(stat = "identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9)) +
  xlab("Trial Type") + 
  ylab("Mean Reaction Time (ms)") 
