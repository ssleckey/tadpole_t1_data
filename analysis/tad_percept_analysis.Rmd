...
```{r}
glm_maximal_mod_trans <- lmer(trans ~ acc_et * similar + age_months +
                           (acc_et * similar | target) +
                           (acc_et *  similar | ID), 
                         data = d_et_filt)
...
```{r}
d_filt$acc <- as.numeric(as.character(d_filt$acc))
...
describeBy(d_filt$RT, d_filt$acc)
```


```{r}
glm_maximal_mod_RT <- lmer(RT ~ acc * similar + age_months +
                           (acc * similar | target) +
                           (acc * similar | ID), 
                         data = d_filt)
