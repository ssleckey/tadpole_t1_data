---
title: "Calculate Fixations and Transitions"
author: "Emily"
date: "9/12/2017"
output: html_document
---

Code to calculate fixations and transitions from ET data in Toddler Uncertainty study. Raw data is exported from Tobii Studio. Used default fixation filters -- velocity threshold: 35 pixels per sample; distance threshold: 35 pixels; minimum fixation duration: 60 ms
...
Read in data and get transitions. Transitions are defined as a switch from one AOI to another. A trial with a fixation to only one AOI has zero transitions.
...
fixationcount <- 0
fixations_id <- data.frame()
trialcount <- 0

for (i in 2:(nrow(removedmid)-1)){
    if (removedmid$Event[i] == 'correct' | removedmid$Event[i] == 'incorrect'){
    fixationcount <- fixationcount + 1
    }

  if (removedmid$Event[i] == 'ImageEnd'){
    trialcount <- trialcount + 1
    trial_fixation <- trialcount
    temp_fixation <- fixationcount
    fixationcount <- 0
    fixes <- data.frame(trial = trialcount,
                        fixations = temp_fixation,
                        SID = SID)
    fixations_id <- bind_rows(fixations_id, fixes)
  }
}
